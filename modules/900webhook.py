...
3. events.<事件名>.aliases: 事件别名列表，大小写不敏感。
4. events.<事件名>.vars: 模板变量提取规则。
   - from: 点路径取值，如 "eventData.name"、"Item.Id"
   - extract: 正则列表，命中后取第一个分组
   - default: 未取到值时的默认值
5. events.<事件名>.require / skip_if: 事件级条件判断。
6. events.<事件名>.send: 发送目标列表，每条消息可单独配置 when 条件。
"""
...
class Webhook(Module):
    ...
    def extract_msg_type(self, data: dict):
        ...
        if status_value:
            ...
            return self.normalize_msg_type(status_event_map.get(status_value, ""))
        return ""
    ...
    def msg_has_reported(self, data: dict, period=86400):
        ...
        if any(msg == i["msg"] for i in self.msg_deque):
            reported = True
        elif (
    ...
    def repeat(self, times=2, msg_type: str | None = None, msg: str | None = None):
        ...
        if len(self.msg_deque) == 0:
            return False
        if ((msg_type and msg_type == self.msg_deque[-1]["type"]) or (msg and msg == self.msg_deque[-1]["msg"])):
            latest_times = 0
            for item in reversed(self.msg_deque):
                if ((msg_type and msg_type == item["type"]) and (msg is None or msg == item["msg"])):
                    latest_times += 1
                else:
                    break
    ...
    def occur(self, msg_type: str, second: int):
        ...
        time.sleep(second)
        new_msg_list = [item for item in self.msg_imm_deque if item not in ori_deque]
        for item in new_msg_list:
            if msg_type == item["type"]:
